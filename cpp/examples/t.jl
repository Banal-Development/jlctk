...
function main()
 ...
 ring_o = view(unsafe_wrap(Vector{RingDescriptor}, mmap_ptr, 1), 1)[1]
 planet_pos_v_addr = reinterpret(Ptr{PlanetPos}, mmap_v_ptr + sizeof(RingDescriptor))
 planet_pos_v = unsafe_wrap(Vector{PlanetPos}, planet_pos_v_addr, ring_o.ring_size)

 println("ring_size: ", ring_o.ring_size)
 println("end_idx: ", ring_o.end_idx) 
 println(mmap_v_ptr, " ", mmap_v_ptr + sizeof(RingDescriptor))
 println(mmap_ptr, " ", mmap_ptr + sizeof(RingDescriptor))

 start_idx = 1
 while true
  last_idx = ring_o.end_idx-1
  if last_idx >= start_idx
     println(planet_pos_v[mod(start_idx, ring_o.ring_size)], " --> ", planet_pos_v[mod(last_idx, ring_o.ring_size)])
     println(String(planet_pos_v[mod(start_idx, ring_o.ring_size)].planet))
  end
